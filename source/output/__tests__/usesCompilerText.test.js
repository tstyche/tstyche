...
import { ProjectConfigKind, Scribbler, usesCompilerText } from "tstyche/tstyche";
...
test("usesCompilerText", async (t) => {
  await t.test("formats uses text of with discovered config", () => {
    ...
    const text = scribbler.render(usesCompilerText("5.3.4", { kind: ProjectConfigKind.Discovered, specifier }));
  ...
  await t.test("formats uses text of with provided config", () => {
    ...
    const text = scribbler.render(usesCompilerText("5.3.4", { kind: ProjectConfigKind.Provided, specifier }));
  ...
  await t.test("formats uses text of with default config", () => {
    const text = scribbler.render(
      usesCompilerText("5.3.4", { kind: ProjectConfigKind.Default, specifier: "baseline" }),
    );
  ...
  await t.test("formats uses text of with inline config", () => {
    ...
    const text = scribbler.render(usesCompilerText("5.3.4", { kind: ProjectConfigKind.Synthetic, specifier }));
  ...
  await t.test("formats uses short text", () => {
    ...
    const text = scribbler.render(
      usesCompilerText("5.3.4", { kind: ProjectConfigKind.Discovered, specifier }, { short: true }),
    );
