...
import { Scribbler, usesText } from "tstyche/tstyche";
...
test("usesText", async (t) => {
  await t.test("formats uses text", () => {
    const text = scribbler.render(usesText("5.3.4", sampleProjectConfigFilePath));
  ...
  await t.test("formats uses text without project config file path", () => {
    const text = scribbler.render(usesText("5.3.4", undefined));

    assert.strictEqual(prettyAnsi(text), ["<blue>uses</> TypeScript 5.3.4", "", ""].join("\n"));
  });

  await t.test("formats uses short text", () => {
    const text = scribbler.render(usesText("5.3.4", sampleProjectConfigFilePath, { short: true }));
