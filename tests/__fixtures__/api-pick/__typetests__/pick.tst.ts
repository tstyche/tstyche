...
test("are several keys picked?", () => {
  expect(pick(returnsObject(), "one", "three")).type.toBe<{ one: number; three?: boolean }>();
  expect(returnsObject()).type.toBe<{ one: number; three?: boolean }>(); // fail

  expect(pick(returnsIntersection(), "one", "three")).type.toBe<{ one: number; three?: boolean }>();
  expect(returnsIntersection()).type.toBe<{ one: number; three?: boolean }>(); // fail

  expect(pick(returnsUnion(), "two", "three")).type.toBe<{ two?: string; three?: boolean }>();
