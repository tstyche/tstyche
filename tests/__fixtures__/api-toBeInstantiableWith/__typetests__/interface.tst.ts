...
interface Single<T extends string = string> {
  value: T;
}

interface Double<T, V> {
  first: T;
  second: V;
}

interface Triple<T extends string, U extends number = number, V = boolean> {
  first: T;
  second: U;
  third: V;
}

describe("when source is an interface", () => {
